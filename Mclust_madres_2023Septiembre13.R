...
#agregar elipses
for (i in 2:Mcluster.phenodata$G){
  points(ellipse(x = Mcluster.phenodata$parameters$variance$sigma[1:2,1:2,i],
                 centre = Mcluster.phenodata$parameters$mean[c(1,2),i], level = pchisq(1, 2)),
         type="l", col="black")
}
...
###################################################################################################################
# 6.6)  Examinar la distribución de Espeletia argentea citado y Espeletia tapirophila tipo
# 6.6.1) Para todas las plantas
#plot in PC3 and PC2
#par(mar=c(5,4,4,2)+0.1) #default
par(mar=c(5,5,4,2)+0.1)
plot(Mcluster.phenodata, what=c("classification"), dimens=c(3,2), main="", addEllipses = F,
...
#agregar título
title(expression(paste("A) Especímenes citados como ", italic("E. argentea"), " y ", italic("E. tapirophila"))), adj=0)

# 6.6.2) Para las plantas madres
#plot in PC3 and PC2
#par(mar=c(5,4,4,2)+0.1) #default
par(mar=c(5,5,4,2)+0.1)
plot(Mcluster.phenodata, what=c("classification"), dimens=c(3,2), main="", addEllipses = F,
...
points(Mcluster.phenodata$data[Mcluster.phenodata$classification==4,3],
       Mcluster.phenodata$data[Mcluster.phenodata$classification==4,2],
...
# título
title(expression(paste("C) Especímenes citados como ", italic("E. grandiflora"))), adj=0)
...
plot(Mcluster.phenodata, what=c("classification"), dimens=c(3,2), main="", addEllipses = F,
     xlab="PC3 (11.47% varianza)", ylab="PC2 (14.81% varianza)", cex=0, cex.lab=1.5, cex.axis=1.5)
# Agregar especímenes asignados al P5
points(Mcluster.phenodata$data[Mcluster.phenodata$classification==5,3],
       Mcluster.phenodata$data[Mcluster.phenodata$classification==5,2],
...
rect(xleft=-2.3, ybottom=-1.8, xright=-1.74, ytop=-2.2)
#add title
title(expression(paste("E) Especímenes citados como ", italic("E. killipii"))), adj=0)
...
###################################################################################################################
# 6.9) Espeletia summapacis tipo y citados
...
points(Mcluster.phenodata$data[Mcluster.phenodata$classification==1,3],
       Mcluster.phenodata$data[Mcluster.phenodata$classification==1,2],
       pch=mclust.options("classPlotSymbols")[1], cex=1, col="gray90")
#add ellipses
for (i in 1:Mcluster.phenodata$G){
  points(ellipse(x = Mcluster.phenodata$parameters$variance$sigma[c(3,2),c(3,2),i],
                 centre = Mcluster.phenodata$parameters$mean[c(3,2),i], level = pchisq(1, 2)),
...
rect(xleft=-2.3, ybottom=-1.8, xright=-1.74, ytop=-2.2)
#add title
title(expression(paste("D) Especímenes citados como ", italic("E. summapacis"))), adj=0)
...
###################################################################################################################
###################################################################################################################
# 7) Examinar la localización Examine the location of specimens similar to type specimens of Espeletia cabrerensis and
#    Espeletia miradorensis in the the best normal mixture model.
###################################################################################################################
###################################################################################################################
setwd("C:/Users/usuario/Documents/Jardin_comun/Figuras")#Directorio de Diana
#setwd("C:/_transfer/Review/MelissaPineda/Data_Melissa") #Ivan's working directory Lehmann
