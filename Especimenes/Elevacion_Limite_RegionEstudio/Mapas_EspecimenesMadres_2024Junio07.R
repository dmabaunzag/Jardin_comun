...
plot(
  ...
  mar = c(5, 4, 2, 7),
  plg = list(cex = 1.5),
  col = hcl.colors(n = 8, palette = "Light Grays"),
  range = c(3250, 4050)
)
axis(1,
     seq(-74.29,-74.25, 0.01),
     cex.axis = 1.5,
     lwd = 2)
axis(
  2,
  at = seq(4.19, 4.22, 0.01),
  cex.axis = 1.5,
  lwd = 2,
  line = -5.6
)
...
plot(
  sv.especimenes,
  col = morfogrupo.color[sv.especimenes$Phenotypic.Group],
  pch = morfogrupo.simbolo[sv.especimenes$Phenotypic.Group],
  cex = 1.5,
  add = T
)
mtext(
...
morfogrupo.color <-
  c("dodgerblue2",
    ...
    "slateblue",
    "darkorange",
    "skyblue1") #definir colores para cada grupo morfológico, de acuerdo a la figuras de Mclust
...
plot(
  sv.especimenes[ipm, ],
  col = morfogrupo.color[sv.especimenes$Phenotypic.Group[ipm]],
  pch = morfogrupo.simbolo[sv.especimenes$Phenotypic.Group[ipm]],
  cex = 1.5,
  add = T
)
mtext(
...
points(sv.especimenes[igm, ],
       col = morfogrupo.color[gm],
       pch = morfogrupo.simbolo[gm],
       cex = 1.5)
